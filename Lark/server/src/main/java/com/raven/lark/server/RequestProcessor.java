...
{
	private LarkServer server;
	private Socket socket;
	RequestProcessor(LarkServer server, Socket socket)
	{
		this.server=server;
		this.socket=socket;
		start();
		...
		try
		{
			...
			else
			{
				...
				}catch(IllegalAccessException illegalAccessException)
				{
					...
					responseObject.setResult(null);
					responseObject.setException(new RuntimeException("Unable to create object to service class associated with the path : "+servicePath));
				}catch(InvocationTargetException inovcationTargetException)
				{
					Throwable t=new Throwable();
					t=inovcationTargetException.getCause();
					responseObject.setSuccess(false);
					responseObject.setResult(null);
